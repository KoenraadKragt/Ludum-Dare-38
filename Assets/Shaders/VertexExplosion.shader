...
Shader "VertexExplosion" {//The Shaders Name
//The inputs shown in the material panel
Properties {
	_SSSAlbedo_aColor ("Albedo - Color", Color) = (0.627451,0.8,0.8823529,1)
	_SSSVertex_aMix_Amount ("Vertex - Mix Amount", Range(0.000000000,1.000000000)) = 0.000000000
}
	...
	//Generate layers for the Vertex channel.
		//Generate Layer: Vertex
			//Sample parts of the layer:
				half4 VertexVertex_Sample1 = v.color;

			//Blend the layer into the channel using the Mix blend mode
				Vertex = lerp(Vertex,((VertexVertex_Sample1)*float4(v.normal.rgb,1)).rgba,_SSSVertex_aMix_Amount);
...
Mask#!S2#^ObjectArray#^-1#^CC0#?Mask
EndShaderInput
ShaderName#!S2#^Text#^VertexExplosion#^CC0#?ShaderName
...
Setting1#!S2#^Float#^0.9042146#^CC0#?Setting1
Wrap Color#!S2#^Vec#^0.4,0.2,0.2,1#^CC0#?Wrap Color
...
Specular Type#!S2#^Type#^0#^CC0#?Specular Type
Spec Hardness#!S2#^Float#^0.8352656#^CC0#?Spec Hardness
...
Real Layer Type#!S2#^Text#^SLTVertexColors#^CC0#?Real Layer Type
Layer Type#!S2#^Type#^0#^CC0#?Layer Type
