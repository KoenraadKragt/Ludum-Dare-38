...
#ifndef VERTEXTOASTIE_STANDARD_CORE_INCLUDED
#define VERTEXTOASTIE_STANDARD_CORE_INCLUDED

#include "UnityCG.cginc"
#include "UnityShaderVariables.cginc"
#include "UnityStandardConfig.cginc"
#include "VertexToastieStandardInput.cginc"
...
half4 fragForwardBase (VertexOutputForwardBase i) : SV_Target
{
	...
	half occlusion = Occlusion(i.tex.xy);
		...
		occlusion *= (1-(testpow((1-i.color.a),_AOPower)*_AOStrength));
	#endif
	...
	#if defined(ApplyAOToDiffuse)
		c.rgb *= (1-(testpow((1-i.color.a),_AOPower)*_AOStrength));
	#endif
	#if defined(Visualize_AO)
		c.rgb = (1-(testpow((1-i.color.a),_AOPower)));
	#endif
	//c.rgb *= (1-((1-i.color.a)*_AOStrength));
...
half4 fragForwardBase (VertexOutputBaseSimple i) : SV_Target
{
	...
	half occlusion = Occlusion(i.tex.xy);
	//occlusion *= (1-((1-i.color.a)*_AOStrength));
	#if !defined(ApplyAOToDiffuse)
		occlusion *= (1-(testpow((1-i.color.a),_AOPower)*_AOStrength));
	#endif
...
struct VertexOutputDeferred
{
	...
	half3 eyeVec 						: TEXCOORD1;
	half4 tangentToWorldAndParallax[3]	: TEXCOORD2;	// [3x3:tangentToWorld | 1x3:viewDirForParallax]
	half4 ambientOrLightmapUV			: TEXCOORD6;	// SH or Lightmap UVs			
	#if UNITY_SPECCUBE_BOX_PROJECTION
...
)
{
	...
	outEmission = half4(color, 1);
	#if defined(ApplyAOToDiffuse)
		outDiffuse *= (1-(testpow((1-i.color.a),_AOPower)*_AOStrength));
		outSpecSmoothness.rgb *= (1-(testpow((1-i.color.a),_AOPower)*_AOStrength));
		outEmission *= (1-(testpow((1-i.color.a),_AOPower)*_AOStrength));
	#else
		outEmission *= (1-(testpow((1-i.color.a),_AOPower)*_AOStrength));
	#endif
	#if defined(Visualize_AO)
		outDiffuse = 0;
		outSpecSmoothness = 0;
		outEmission = (1-(testpow((1-i.color.a),_AOPower)));
	#endif
	#ifndef UNITY_HDR_ON
		outEmission.rgb = exp2(-outEmission.rgb);
	#endif
	//outDiffuse *= (1-((1-i.color.a)*_AOStrength));
}
