...
public class ShaderSandwich : EditorWindow{//, ISerializationCallbackReceiver {
	...
	public void Ser(){
		...
		if (EditorApplication.isPlaying&&EditorApplication.isPlayingOrWillChangePlaymode&&!EditorApplication.isPaused){//In Play Mode
			...
			LoadFromSerializedProtectStuff();
			SerializeProtection = "";
		}
...
//		Debug.Log("It's ok.");
	...
	public void LoadFromSerializedProtectStuff(){
...
//		Debug.Log((Texture2D)AssetDatabase.LoadAssetAtPath("Assets/ElectronicMindStudios/Shader Sandwich/Internal/Shader Sandwich/Textures/ShaderSandwichPerlinNoise.png",typeof(Texture2D)));
	...
	if (SSSettings==null){
		SSSettings = (ShaderSandwichSettings)AssetDatabase.LoadAssetAtPath("Assets/ElectronicMindStudios/Shader Sandwich/Internal/Editor/Shader Sandwich/SSSettings.asset",typeof(ShaderSandwichSettings));
		if (SSSettings==null){
			SSSettings = (ShaderSandwichSettings)ScriptableObject.CreateInstance(typeof(ShaderSandwichSettings));
			AssetDatabase.CreateAsset(SSSettings,"Assets/ElectronicMindStudios/Shader Sandwich/Internal/Editor/Shader Sandwich/SSSettings.asset");
		}
	}
...
//		GUISkin oldskin = GUI.skin;
		...
		if (OpenShader!=null&&AnimateInputs&&Event.current.type==EventType.Repaint){
			...
			if (OpenShader!=null&&OpenShader.ShaderPasses!=null&&OpenShader.ShaderPasses.Count>0&&CurInterfaceStyle==InterfaceStyle.Modern){
				...
				if (GUI.Button( EMindGUI.Rect2(RectDir.Bottom,0,WinSize.y,58,30),"Home",ButtonStyle)&&GUIStage!=GUIType.Start)
					Goto(GUIType.Start,ShaderTransition.TransDir.Backward);
				if (GUI.Button( EMindGUI.Rect2(RectDir.Bottom,58,WinSize.y,68,30),"Layers",ButtonStyle)&&GUIStage!=GUIType.Layers)
					Goto(GUIType.Layers,(GUITransition>GUIType.Layers)?ShaderTransition.TransDir.Backward:ShaderTransition.TransDir.Forward);	
				if (GUI.Button( EMindGUI.Rect2(RectDir.Bottom,58+68,WinSize.y,72,30),"Settings",ButtonStyle)&&GUIStage!=GUIType.Configure)
				...
				if (GUI.Button( EMindGUI.Rect2(RectDir.Bottom,58+68+72,WinSize.y,67,30),"Inputs",ButtonStyle)&&GUIStage!=GUIType.Inputs)
...
//			Debug.Log(GUITransition);
			...
			if (OpenShader!=null&&CurInterfaceStyle==InterfaceStyle.Modern){
				GUIStyle ButtonStyle;
				if(CurInterfaceStyle==InterfaceStyle.Flat||CurInterfaceStyle==InterfaceStyle.Modern){
					ButtonStyle = new GUIStyle(EMindGUI.SCSkin.window);
					ButtonStyle.alignment = TextAnchor.MiddleCenter;
				}
				else
					ButtonStyle = new GUIStyle(GUI.skin.button);
				...
				GUI.Toggle( EMindGUI.Rect2(RectDir.Bottom,1,WinSize.y,66,30),GUITransition==GUIType.Start,"Home",ButtonStyle);
				GUI.Toggle( EMindGUI.Rect2(RectDir.Bottom,67,WinSize.y,66,30),GUITransition==GUIType.Layers,"Layers",ButtonStyle);
				GUI.Toggle( EMindGUI.Rect2(RectDir.Bottom,67+66,WinSize.y,66,30),GUITransition==GUIType.Configure,"Passes",ButtonStyle);
				GUI.Toggle( EMindGUI.Rect2(RectDir.Bottom,67+66+66,WinSize.y,66,30),GUITransition==GUIType.Inputs,"Inputs",ButtonStyle);
	...
	Vector2 GUIConfigureBoxStart(string Name,ShaderVar Tickbox,Vector2 WinSize,float X,float Y){
		...
		ShaderUtil.BeginGroup(new Rect((int)((BoxWidth)*(float)X+15f),(BoxHeight)*(float)Y+15f+144f+BaseSettingsHeight,BoxWidth-15f,BoxHeight-15f),GUI.skin.box);
		else
		ShaderUtil.BeginGroup(new Rect((int)((BoxWidth)*(float)X+15f),(BoxHeight)*(float)Y+15f+144f+BaseSettingsHeight,BoxWidth-15f,BoxHeight-15f),GUI.skin.button);
	...
	void GUIConfigure(Vector2 WinSize,Vector2 Position)
	{
		...
		else
		{
			...
			if (GUI.Button( EMindGUI.Rect2(RectDir.Diag,WinSize.x,WinSize.y,100,30),"Layers",ButtonStyle))
			...
			//ConfigScroll = ShaderUtil.BeginScrollView(new Rect(0,0,WinSize.x,WinSize.y),ConfigScroll,new Rect(0,0,WinSize.x-16,(BoxSize.y+15f)*6.2f+144f+29f+BaseSettingsHeight),false,true);
			ConfigScroll = ShaderUtil.BeginScrollView(new Rect(0,0,WinSize.x,WinSize.y),ConfigScroll,new Rect(0,0,WinSize.x-16,height),false,true);
			...
			if (CurInterfaceStyle==InterfaceStyle.Flat)
			GUI.Box(new Rect(16,75,WinSize.x-32,BaseSettingsHeight-15),"");
			else
			GUI.Box(new Rect(16,75,WinSize.x-32,BaseSettingsHeight-15),"","button");
			...
			GUI.Box(new Rect(15,115+BaseSettingsHeight,WinSize.x-25,45),"","button");
			GUI.skin.label.fontSize = 18;
			GUI.Label(new Rect(25,125+BaseSettingsHeight,WinSize.x-25,25),"Pass Group Name: ");
			TextFieldStyle.fontSize = 18;
			
			SP.Name.Text = GUI.TextField(new Rect(200,125+BaseSettingsHeight,150,25),SP.Name.Text,TextFieldStyle);
			GUIConfigureBoxStart("Diffuse (Lighting)",SP.DiffuseOn,WinSize,0,0);
			
			SP.DiffuseLightingType.Draw(new Rect(0,30,124,124));
			
			EMindGUI.Label(new Rect(122,30,BoxSize.x-124,70),SP.DiffuseLightingType.Descriptions[SP.DiffuseLightingType.Type],12);
			
			if (SP.DiffuseLightingType.Type==0){
				GUI.Label(new Rect(122,80,BoxSize.x-124,20),"Quality: ");
				SP.PBRQuality.Draw(new Rect(122+49,80,BoxSize.x-124-49,20),"Quality: ");
				GUI.Label(new Rect(122,105,BoxSize.x-124,20),"Model: ");
				SP.PBRModel.Draw(new Rect(122+49,100,BoxSize.x-124-49,20),"Model: ");
			}
			...
			SP.SpecularLightingType.Draw(new Rect(0,30,124,124));
			EMindGUI.Label(new Rect(122,30,BoxSize.x-124,70),SP.SpecularLightingType.Descriptions[SP.SpecularLightingType.Type],12);
			if (SP.IsPBR)
				GUI.enabled = true;
			}
			else
			SP.SpecularLightingType.DrawPicType(new Rect(0,30,124,124),SP.DiffuseLightingType.GetImage(0),"Off");
			
			
			SP.SpecularHardness.Range0 = 0.0001f;
			SP.SpecularHardness.Draw(new Rect(122,60,BoxSize.x-124,20),"Smoothness: ");
			SP.MaxLightSpecularHardness.Draw(new Rect(122,80,BoxSize.x-124,20),"Min Sharpness: ");
			if (SP.IsPBR){
				GUI.enabled = false;
			...
			if (SP.EmissionOn.On){
			SP.EmissionType.Draw(new Rect(0,30,124,124));
			EMindGUI.Label(new Rect(122,30,BoxSize.x-124,70),SP.EmissionType.Descriptions[SP.EmissionType.Type],12);
			}
			else
			SP.OtherTypes.DrawPicType(new Rect(0,30,124,124),SP.OtherTypes.GetImage(3),"Off");
			...
			SP.TransparencyType.Draw(new Rect(0,30,124,124));
			EMindGUI.Label(new Rect(122,30,BoxSize.x-124,70),SP.TransparencyType.Descriptions[SP.TransparencyType.Type],12);
			}
			else
			SP.OtherTypes.DrawPicType(new Rect(0,30,124,124),SP.OtherTypes.GetImage(2),"Off");

			if (SP.TransparencyType.Type==0)//||(SP.TransparencyZWrite.On&&SP.TransparencyZWriteType.Type==1))
			SP.TransparencyAmount.Draw(new Rect(122,80,BoxSize.x-124,20),"Cutoff: ");
			if (SP.TransparencyType.Type==1)//&&!(SP.TransparencyZWrite.On&&SP.TransparencyZWriteType.Type==1))
			SP.TransparencyAmount.Draw(new Rect(122,80,BoxSize.x-124,20),"Transparency: ");
			...
			//SP.TransparencyZWrite.Draw(new Rect(122,130,BoxSize.x-124,20),"Z Write: ");
			
			//if (!SP.TransparencyZWrite.On)
			//GUI.enabled = false;
			GUI.Label(new Rect(122+150,130,BoxSize.x-124-150,20),"Z Write: ");
			SP.TransparencyZWriteType.Draw(new Rect(122+150+50,130,BoxSize.x-124-150-50,20),"");
			
			GUI.enabled = OldGUIEnabled;
			
			SP.TransparencyCorrectShadows.Draw(new Rect(122,105,BoxSize.x-124,20),"Correct Shadows: ");
			
			if (!(SP.IsPBR)||!(SP.TransparencyType.Type==1)){
				SP.TransparencyPBR.On = false;
				GUI.enabled = false;
			}
			SP.TransparencyPBR.Draw(new Rect(122,130,BoxSize.x-124,20),"Use PBR: ");
			...
			GUI.Label(new Rect(122+150,155,AHalf,20),"Blend: ");
			SP.BlendMode.Draw(new Rect(122+200,155,BoxSize.x-(122+202),20),"Blend Modes: ");
			
			SP.TransparencyPreMult2.Draw(new Rect(122,155,BoxSize.x-124,20),"Pre-Multiply: ");
			
			GUI.enabled = OldGUIEnabled;
			
			if (SP.TransparencyType.Type==0)
				SP.TransparencyAlphaToCoverage.Draw(new Rect(122+150,105,BoxSize.x-124-150,20),"Alpha To Coverage: ");
			else
			if (SP.TransparencyZWriteType.Type==2||SP.TransparencyCorrectShadows.On){
				//GUI.enabled = true;
				SP.TransparencyZWriteAmount.LabelOffset = 80;
				SP.TransparencyZWriteAmount.Draw(new Rect(122+150,105,BoxSize.x-124-150,20),"Cutoff: ");
			}
			...
			if (SP.ShellsOn.On){
			SP.OtherTypes.DrawPicType(new Rect(0,30,124,124),SP.OtherTypes.GetImage(5),"On",false);
			//EMindGUI.Label(new Rect(122,30,BoxSize.x-124,70),SP.TransparencyType.Descriptions[SP.TransparencyType.Type],12);
			}
			else
			SP.OtherTypes.DrawPicType(new Rect(0,30,124,124),SP.OtherTypes.GetImage(4),"Off",false);		
			...
			{
			SP.OtherTypes.DrawPicType(new Rect(0,30,124,124),SP.OtherTypes.GetImage(1),"On");
			}
			else
			SP.OtherTypes.DrawPicType(new Rect(0,30,124,124),SP.OtherTypes.GetImage(0),"Off");
			...
			SP.ParallaxDepthCorrection.Draw(new Rect(122+150,90,BoxSize.x-124,20),"Correct Intersections: ");
			...
			{
			SP.OtherTypes.DrawPicType(new Rect(0,30,124,124),SP.OtherTypes.GetImage(7),"On");
			}
			else
			SP.OtherTypes.DrawPicType(new Rect(0,30,124,124),SP.OtherTypes.GetImage(6),"Off");
			...
			float AdditionalHeight = 4f;
//			Debug.Log(ShaderSandwichPokeInstalled);
			if (ShaderSandwichPokeInstalled){
				AdditionalHeight += 1.2f;
				GUI.Box(new Rect((int)((BoxSize.x+15f)*(float)0f+15f),(BoxSize.y+15f)*(float)AdditionalHeight+15f+144f+BaseSettingsHeight-32f,WinSize.x-30f,33),"","button");
				GUI.skin.label.fontSize = 18;
				GUI.Label(new Rect((int)((BoxSize.x+15f)*(float)0f+15f)+10f,(BoxSize.y+15f)*(float)AdditionalHeight+15f+144f+BaseSettingsHeight-31f+5f,WinSize.x-30f,32),"Shader Sandwich Extras");
				EMindGUI.SetLabelSize(12);
				GUIConfigureBoxStart("Poke",SP.PokeOn,WinSize,0,AdditionalHeight);
				float AHalf2 = (BoxSize.x-20)/2;
					SP.PokeCount.Range1 = 5;
					SP.PokeCount.NoInputs = true;
					SP.PokeCount.Float = Mathf.Round(SP.PokeCount.Float);
					SP.PokeLine.Draw(new Rect(10,110,BoxSize.x-20,20),"Lines Enabled:");
					SP.PokeCustom.Draw(new Rect(10+AHalf2,110,BoxSize.x-20,20),"Custom Channels:");
					SP.PokeCount.Draw(new Rect(10,85,BoxSize.x-20,20),"Maximum Poke Count:");
					SP.PokeInteraction.TypeDispL = 2;
					SP.PokeInteraction.Draw(new Rect(10,40,BoxSize.x-20,20),"Poke Interaction:");
					
					
					SP.PokeFloats.Float = Mathf.Round(SP.PokeFloats.Float);
					SP.PokeFloats.NoSlider = true;
					SP.PokeFloats.NoArrows = true;
					SP.PokeFloats.Draw(new Rect(10,140,AHalf2,20),"Additional Floats:");
					SP.PokeFloat3s.Float = Mathf.Round(SP.PokeFloat3s.Float);
					SP.PokeFloat3s.NoSlider = true;
					SP.PokeFloat3s.NoArrows = true;
					SP.PokeFloat3s.Draw(new Rect(AHalf2+10,140,AHalf2,20),"Additional Float3s:");
					
				GUIConfigureBoxEnd();
			}
			if (VertexToastieInstalled){
				AdditionalHeight += 1.2f;
				GUI.Box(new Rect((int)((BoxSize.x+15f)*(float)0f+15f),(BoxSize.y+15f)*(float)AdditionalHeight+15f+144f+BaseSettingsHeight-32f,WinSize.x-30f,33),"","button");
				GUI.skin.label.fontSize = 18;
				GUI.Label(new Rect((int)((BoxSize.x+15f)*(float)0f+15f)+10f,(BoxSize.y+15f)*(float)AdditionalHeight+15f+144f+BaseSettingsHeight-31f+5f,WinSize.x-30f,32),"Vertex Toastie Stuff");
				EMindGUI.SetLabelSize(12);
				GUIConfigureBoxStart("Ambient Occlusion",SP.IndirectAmbientOcclusionOn,WinSize,0,AdditionalHeight);
					if (SP.TessellationOn.On==true&&SP.TessellationQuality.Float>0)
					{
						SP.OtherTypes.DrawPicType(new Rect(0,30,124,124),SP.OtherTypes.GetImage(7),"On");
					}
					...
					//SP.IndirectAmbientOcclusion.TypeDispL = 3;
					SP.IndirectAmbientOcclusionControl.Draw(new Rect(122,30,BoxSize.x-124,20),"Enabled:");
					//SP.IndirectAmbientOcclusionRealtime.Draw(new Rect(122,50,BoxSize.x-124-(int)((BoxSize.x-124)/2),20),"Enabled:");
					SP.IndirectAmbientOcclusionRealtime.Draw(new Rect(122,50,BoxSize.x-124-(int)((BoxSize.x-124)/2),20),"Enabled:");
					
					SP.IndirectAmbientOcclusionOptimized.Draw(new Rect(122+(int)((BoxSize.x-124)/2),50,BoxSize.x-124-(int)((BoxSize.x-124)/2),20),"Optimized:");
					bool OldGuiEna = GUI.enabled;
					GUI.enabled = SP.IndirectAmbientOcclusionOptimized.On;
					SP.IndirectAmbientOcclusionOptimizedType.Draw(new Rect(122+(int)((BoxSize.x-124)/2),70,BoxSize.x-124-(int)((BoxSize.x-124)/2),20),"Optimized:");
					GUI.enabled = OldGuiEna;
					
					OldGUIEnabled = GUI.enabled;
					GUI.enabled = GUI.enabled&&!(SP.IndirectAmbientOcclusionControl.Type==0);
					SP.IndirectAmbientOcclusionStrength.Range1 = 3f;
					SP.IndirectAmbientOcclusionStrength.Draw(new Rect(122,95,BoxSize.x-124,20),"Strength:");
					
					SP.IndirectAmbientOcclusionPushback.Range1 = 4f;
					SP.IndirectAmbientOcclusionPushback.Draw(new Rect(122,120,BoxSize.x-124,20),"Thinness:");
					
					SP.IndirectAmbientOcclusionDisplayType.Draw(new Rect(122,145,BoxSize.x-124,20),"Display:");
					
					GUI.enabled = OldGUIEnabled;
					
				GUIConfigureBoxEnd();
				GUIConfigureBoxStart("Subsurface Scattering",SP.IndirectSubsurfaceScatteringOn,WinSize,1,AdditionalHeight);
					//if (SP.TessellationOn.On==true&&SP.TessellationQuality.Float>0)
					//{
					//	SP.OtherTypes.DrawPicType(new Rect(0,30,124,124),SP.OtherTypes.GetImage(7),"On");
					//}
					...
					SP.IndirectSubsurfaceScattering.TypeDispL = 2;
					SP.IndirectSubsurfaceScatteringUseThickness.Draw(new Rect(12,30,(BoxSize.x-14)/2,20),"Use Thickness:");
					OldGUIEnabled = GUI.enabled;
					GUI.enabled = GUI.enabled&&SP.IndirectSubsurfaceScatteringUseThickness.On;
					SP.IndirectSubsurfaceScattering.Draw(new Rect(12+(int)((BoxSize.x-14)/2),30,(int)(BoxSize.x-(BoxSize.x-14)/2),20),"Enabled:");
					GUI.enabled = OldGUIEnabled;
					SP.IndirectSubsurfaceScatteringMix.Draw(new Rect(12,50,BoxSize.x-14,20),"Translucency:");
					SP.IndirectSubsurfaceScatteringDensity.Range0 = 1f;
					SP.IndirectSubsurfaceScatteringDensity.Range1 = 100f;
					SP.IndirectSubsurfaceScatteringDensity.Draw(new Rect(12,75,BoxSize.x-14,20),"Density:");
					SP.IndirectSubsurfaceScatteringAmbient.Draw(new Rect(12,100,BoxSize.x-14,20),"Ambient:");
					
					SP.IndirectSubsurfaceScatteringScale.Range1 = 32f;
					SP.IndirectSubsurfaceScatteringScale.Draw(new Rect(12,150,(BoxSize.x-14)/2,20),"Fresnel Strength:");
					SP.IndirectSubsurfaceScatteringThinness.Range0 = 1f;
					SP.IndirectSubsurfaceScatteringThinness.Range1 = 32f;
					SP.IndirectSubsurfaceScatteringDistortion.Draw(new Rect(12+(BoxSize.x-14)/2,150,(BoxSize.x-14)/2,20),"Fresnel Distortion:");
					
					SP.IndirectSubsurfaceScatteringThinness.Draw(new Rect(12,125,BoxSize.x-14,20),"Fresnel Thinness:");
					
				GUIConfigureBoxEnd();
			}
			
			GUIConfigureBoxStart("Misc Lighting Settings",null,WinSize,1,3);
				
				SP.MiscAmbient.LabelOffset = (int)AThird+20;
				SP.MiscVertexLights.LabelOffset = (int)AThird+20;
				SP.MiscLightmap.LabelOffset = (int)AThird+20;
				SP.MiscFog.LabelOffset = (int)AThird+20;
				SP.MiscFullShadows.LabelOffset = (int)AThird+20;
				SP.MiscInterpolateView.LabelOffset = (int)AThird+20;
				SP.MiscRecalculateAfterVertex.LabelOffset = (int)AThird+20;
				SP.MiscVertexToastie.LabelOffset = (int)AThird+20;
				SP.MiscVertexToastieOptimized.LabelOffset = (int)AThird+20;
				SP.MiscShadows.LabelOffset = (int)AThird+20;
				SP.MiscForwardAdd.LabelOffset = (int)AThird+20;
				SP.MiscForward.LabelOffset = (int)AThird+20;
				SP.MiscDeferred.LabelOffset = (int)AThird+20;
				SP.MiscShadowCaster.LabelOffset = (int)AThird+20;
				...
				//SP.MiscVertexToastie.Draw(new Rect(12+AThird,30,AThird,20),"Vertex Toastie AO: ");
				
				//SP.MiscShadows.Draw(new Rect(10,150,AThird,20),"Shadows: ");
				EMindGUI.BoldText();
					EMindGUI.Label(new Rect(12,30,200,20),"Lighting Passes: ",12);
				EMindGUI.UnboldText();
				SP.MiscForward.Draw(new Rect(20,50,AHalf,20),"Forward: ");
				SP.MiscForwardAdd.Draw(new Rect(20,70,AHalf,20),"Forward Add: ");
				SP.MiscDeferred.Draw(new Rect(20,90,AHalf,20),"Deferred: ");
				SP.MiscShadowCaster.Draw(new Rect(20,110,AHalf,20),"Shadow Caster: ");
				
				EMindGUI.BoldText();
					EMindGUI.Label(new Rect(12+AHalf,30,200,20),"Lighting Modifiers: ",12);
				EMindGUI.UnboldText();
				SP.MiscAmbient.Draw(new Rect(20+AHalf,50,AHalf,20),"Ambient: ");
				SP.MiscVertexLights.Draw(new Rect(20+AHalf,70,AHalf,20),"Vertex Lights: ");
				SP.MiscLightmap.Draw(new Rect(20+AHalf,90,AHalf,20),"Lightmaps: ");
				SP.MiscShadows.Draw(new Rect(20+AHalf,110,AHalf,20),"Recieve Shadows: ");
				
				OldGUIEnabled = GUI.enabled;
					GUI.enabled = SP.MiscShadows.On;
					if (!SP.MiscShadows.On)
					SP.MiscFullShadows.On = false;
					SP.MiscFullShadows.Draw(new Rect(20+AHalf,130,AHalf,20),"All Forward Shadows: ");
				GUI.enabled = OldGUIEnabled;
				
				//GUI.Box(new Rect(20,150,BoxSize.x-40,21),"","button");
				//SP.MiscVertexToastie.Draw(new Rect(20,152,AHalf,20),"Vertex Toastie AO: ");
				//SP.MiscVertexToastieOptimized.Draw(new Rect(20+AHalf,152,AHalf,20),"AO Optimizations: ");
			...
			GUIConfigureBoxStart("Misc",null,WinSize,0,4);
				SP.TechZTest.TypeDispL = 4;
				
				SP.TechZWrite.Draw(new Rect((int)AThird,50,BoxSize.x-(int)AThird-10,20),"ZWrite: ");
				GUI.Label(new Rect(10,50,BoxSize.x-(int)AThird-10,50),"ZWrite Mode: ");
				
				
				SP.TechZTest.Draw(new Rect((int)AThird,75,BoxSize.x-(int)AThird-10,20),"ZTest: ");
				GUI.Label(new Rect(10,75,BoxSize.x-(int)AThird-10,50),"ZTest Mode: ");
				...
				SP.TechShaderTarget.LabelOffset = (int)AThird-10;
				SP.TechShaderTarget.Draw(new Rect(10,30,BoxSize.x-20,20),"Shader Model: ");
				...
				SP.MiscVertexToastie.LabelOffset = (int)AThird;
				SP.MiscShadows.LabelOffset = (int)AThird;
				SP.MiscForwardAdd.LabelOffset = (int)AThird;
				
				GUI.Label(new Rect(10,120-InterfaceSize,BoxSize.x-(int)AThird-10,50),"Inverse Culling:\n(Sides to show): ");
				SP.TechCull.Draw(new Rect((int)AThird,120,BoxSize.x-(int)AThird-10,20),"Cull: ");
				
				SP.MiscFog.Draw(new Rect(10+AThird,150,AThird,20),"Fog: ");
				SP.MiscInterpolateView.Draw(new Rect(10,150,AThird,20),"Interpolate View: ");
				SP.MiscRecalculateAfterVertex.Draw(new Rect(10+AThird*2,150,AThird,20),"Recalc after Vertex: ");
				
				OldGUIEnabled = GUI.enabled;
			GUIConfigureBoxEnd();
			
			ShaderUtil.EndScrollView();
			EMindGUI.VerticalScrollbar(new Rect(0,0,WinSize.x,WinSize.y),ConfigScroll,new Rect(0,0,WinSize.x-16,height));
	...
	void GUIStart(Vector2 WinSize,Vector2 Position)
	{
		...
		if (CurInterfaceStyle==InterfaceStyle.Modern){
			ButtonStyle2 = new GUIStyle(EMindGUI.SCSkin.window);
		...
		if (CurInterfaceStyle==InterfaceStyle.Modern){
			...
			ButtonStyle2.normal.textColor = EMindGUI.BaseInterfaceColor;
			ButtonStyle2.active.textColor = EMindGUI.BaseInterfaceColor;
			ButtonStyle2.hover.textColor = EMindGUI.BaseInterfaceColor;
			...
			foreach(string File in SSSettings.RecentFilesList){
				if (File!=""&&File!=null){
					...
					if (TheActualFile!=null){
						...
						EMindGUI.Label(new Rect(10,(BannerHeight)+Y+BoxHeight-GUI.skin.label.fontSize-5,RecentlyOpenedSize,BoxHeight),TheActualFile.name+" - "+file.Name,14);
						Y+=BoxHeight;
						Count++;
					}
				}
		...
		GUI.Box(EMindGUI.Rect2(RectDir.Diag,WinSize.x,WinSize.y,RecentlyOpenedSize,WinSize.y-(BannerHeight)),MOTD,LabelStyle);
	...
	bool GUILayersBox(ShaderLayerList list, int X,Vector2 WinSize,GUIStyle ButtonStyle){
		...
		list.Scroll = ShaderUtil.BeginScrollView(new Rect(0,HeightOfLayerName,150,WinSize.y-HeightOfLayerName-100-WidthOfAdd),list.Scroll,new Rect(0,0,130,list.Count*110-10),false,false);
			...
			foreach(ShaderLayer SL in list)
			{
				...
				if (ViewLayerNames)
				EMindGUI.Label(new Rect(30-(15),Pos*110,90,90),SL.Name.Text,11);
				GUI.skin.label.alignment = TextAnchor.UpperLeft;
		...
		ShaderUtil.EndScrollView();
		EMindGUI.VerticalScrollbar(new Rect(0,HeightOfLayerName,150,WinSize.y-HeightOfLayerName-100-WidthOfAdd),list.Scroll,new Rect(0,0,130,list.Count*110-10));
			for(ButtonStyle.fontSize=20;ButtonStyle.fontSize>10;ButtonStyle.fontSize-=1){
				if (ButtonStyle.CalcSize(new GUIContent(list.Name.Text)).x<=130)
				break;
			}
			GUI.Box(new Rect(0,0,150,HeightOfLayerName),"",ButtonStyle);
			if (OpenShader.ShaderLayersMasks.Contains(list)){
				if(CurInterfaceStyle==InterfaceStyle.Modern)
				list.Name.Text = GUI.TextField(new Rect(10,0,130,HeightOfLayerName),list.Name.Text,ButtonStyle);
				else
				list.Name.Text = GUI.TextField(new Rect(10,10,130,HeightOfLayerName-20),list.Name.Text,ButtonStyle);
			}
			else
			...
			if (OpenShader.ShaderLayersMasks.Contains(list)||list==OpenShader.ShaderLayersMaskTemp){
				...
				"a","rgba"};
					...
					EMindGUI.MakeTooltip(0,new Rect(0,WinSize.y-124,24,24),"Delete the Mask (You can't undo this!).");
					if (GUI.Button(new Rect(0,WinSize.y-124,24,24),CrossRed))
					{
						if (EditorUtility.DisplayDialog("Delete Mask?", "Are you sure you want to delete this mask? It could be in use by other layers and effects. Deleting it can't be undone!", "Yes", "No")){
							OpenShader.ShaderLayersMasks.Remove(list);
			...
			if (list.EndTag.Text.Length==1){//||OpenShader.ShaderLayersMasks.Contains(list)){
				...
				if (CurInterfaceStyle == InterfaceStyle.Modern)
				list.EndTag.Text = ColorTag[GUI.SelectionGrid(EMindGUI.Rect2(RectDir.Bottom,1,WinSize.y-100,150-33,24),ColorSelection,new string[]{"R","G","B","A"},4)];
				else
				list.EndTag.Text = ColorTag[GUI.SelectionGrid(EMindGUI.Rect2(RectDir.Bottom,1,WinSize.y-100,150-33,32),ColorSelection,new string[]{"R","G","B","A"},4)];
		...
		if (Event.current.button == 1&&EMindGUI.MouseDownIn(new Rect(0,0,ShaderUtil.GetGroupRect().width,ShaderUtil.GetGroupRect().height-100-30)) ){
			GenericMenu toolsMenu = new GenericMenu();
	...
	void GUILayers(Vector2 WinSize,Vector2 Position)
	{
		...
		else
		{
			...
			if (SP!=null)
			{
				...
				if (SP.SpecularOn.On==true){
					Pos+=1;
					if (SP.UsingMetalModel){
						if(GUILayersBox(SP.ShaderLayersMetallic,Pos,WinSize,ButtonStyle))Hit = true;
						SLList.Add(SP.ShaderLayersMetallic.SLs);
					}
					else{
				...
				if (SP.IndirectSubsurfaceScatteringOn.On==true){
					Pos+=1;
					if(GUILayersBox(SP.ShaderLayersSubsurfaceScattering,Pos,WinSize,ButtonStyle))Hit = true;
					SLList.Add(SP.ShaderLayersSubsurfaceScattering.SLs);
				}
				if (SP.EmissionOn.On==true){
					Pos+=1;
				...
				if (SP.PokeOn.On==true&&SP.PokeCustom.On){
					Pos+=1;
					if(GUILayersBox(SP.ShaderLayersFragmentPoke,Pos,WinSize,ButtonStyle))Hit = true;
					SLList.Add(SP.ShaderLayersFragmentPoke.SLs);
					Pos+=1;
					if(GUILayersBox(SP.ShaderLayersVertexPoke,Pos,WinSize,ButtonStyle))Hit = true;
					SLList.Add(SP.ShaderLayersVertexPoke.SLs);
				}
			...
			if (CurInterfaceStyle!=InterfaceStyle.Modern){
				if (GUI.Button( EMindGUI.Rect2(RectDir.Bottom,0,WinSize.y,64,30),"Back",ButtonStyle))
					Goto(GUIType.Start,ShaderTransition.TransDir.Backward);
				if (GUI.Button( EMindGUI.Rect2(RectDir.Bottom,64,WinSize.y,110,30),"Settings",ButtonStyle))
					Goto(GUIType.Configure,ShaderTransition.TransDir.Backward);	
				if (GUI.Button( EMindGUI.Rect2(RectDir.Bottom,174,WinSize.y,100-24,30),"Inputs",ButtonStyle))
...
//			Debug.Log("Updating Inputs");
	...
	void GUIPresets(Vector2 WinSize,Vector2 Position)
	{
		...
		if (PresetsGUIContents==null||PresetsGUIContents.Count==0||PresetsPaths==null||PresetsPaths.Count==0){
			...
			string[] guids2 = AssetDatabase.FindAssets ("t:Shader", new string[]{"Assets/ElectronicMindStudios/Shader Sandwich/Internal/Editor/Shader Sandwich/Presets"});
