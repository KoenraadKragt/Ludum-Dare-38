	...
	public string Get(float Wrap){
		...
		if (Input!=null)
		{
			if (Input.SpecialType!=InputSpecialTypes.None&&!Input.InEditor){
				UpdateToInput(false);
				string[] SInputTypes = new string[]{"None",Input.CustomSpecial.Text,"_SSTime.y","_SSTime.z","_SSTime.y/2","_SSTime.y/6","_SSSinTime.w","sin(_SSTime.z)","_SSSinTime.z","_SSCosTime.w","cos(_SSTime.z)","_SSCosTime.z","ShellDepth","d.Depth","((_SSSinTime.w+1)/2)","((sin(_SSTime.z)+1)/2)","((_SSSinTime.z+1)/2)","((_SSCosTime.w+1)/2)","((cos(_SSTime.z)+1)/2)","((_SSCosTime.z+1)/2))"};
				if (SG!=null&&!SG.Temp)
				SInputTypes = new string[]{"None",Input.CustomSpecial.Text,"_Time.y","_Time.z","_Time.y/2","_Time.y/6","_SinTime.w","sin(_Time.z)","_SinTime.z","_CosTime.w","cos(_Time.z)","_CosTime.z","ShellDepth","d.Depth","((_SinTime.w+1)/2)","((sin(_Time.z)+1)/2)","((_SinTime.z+1)/2)","((_CosTime.w+1)/2)","((cos(_Time.z)+1)/2)","((_CosTime.z+1)/2)"};
		...
		if (SG.Temp&&ShaderSandwich.Instance.ImprovedUpdates&&((CType==Types.Float&&!NoInputs)||(CType==Types.Vec)))
	...
	bool Draw_Real(Rect rect,DrawTypes d,string S)
	{
		...
		if (d == DrawTypes.Type)
		{
			if (ImagePaths==null||ImagePaths.Length==0){
				...
				ButtonStyle = ShaderSandwich.SCSkin.window;//new GUIStyle(ShaderSandwich.SCSkin.window);
