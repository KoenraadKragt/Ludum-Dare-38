...
public bool IsPBR{
	...
	public void Prepare(){
		...
		ShaderLayersLightingAmbient = new ShaderLayerList("LightingIndirect","Ambient","Customize the ambient lighting.","Lighting","c","rgba","",new Color(0.8f,0.8f,0.8f,1f));
	...
	public List<List<ShaderLayer>> GetShaderLayers(){
		...
		foreach(ShaderPass SP in ShaderPasses){
			tempList.Add(SP.ShaderLayersDiffuse.SLs);
			tempList.Add(SP.ShaderLayersAlpha.SLs);
			tempList.Add(SP.ShaderLayersSpecular.SLs);
			tempList.Add(SP.ShaderLayersSubsurfaceScattering.SLs);
			tempList.Add(SP.ShaderLayersMetallic.SLs);
			tempList.Add(SP.ShaderLayersNormal.SLs);
			tempList.Add(SP.ShaderLayersEmission.SLs);
			tempList.Add(SP.ShaderLayersHeight.SLs);
			tempList.Add(SP.ShaderLayersVertex.SLs);
			tempList.Add(SP.ShaderLayersLightingDiffuse.SLs);
			tempList.Add(SP.ShaderLayersLightingSpecular.SLs);
			tempList.Add(SP.ShaderLayersLightingAmbient.SLs);
			tempList.Add(SP.ShaderLayersLightingAll.SLs);
			tempList.Add(SP.ShaderLayersFragmentPoke.SLs);
			tempList.Add(SP.ShaderLayersVertexPoke.SLs);
		}
		return tempList;
	}	
	public List<ShaderLayerList> GetShaderLayerLists(){
		...
		tempList.Add(SLL);		
		foreach(ShaderPass SP in ShaderPasses){
			tempList.Add(SP.ShaderLayersDiffuse);
			tempList.Add(SP.ShaderLayersAlpha);
			tempList.Add(SP.ShaderLayersSpecular);
			tempList.Add(SP.ShaderLayersSubsurfaceScattering);
			tempList.Add(SP.ShaderLayersMetallic);
			tempList.Add(SP.ShaderLayersNormal);
			tempList.Add(SP.ShaderLayersEmission);
			tempList.Add(SP.ShaderLayersHeight);
			tempList.Add(SP.ShaderLayersVertex);
			tempList.Add(SP.ShaderLayersLightingDiffuse);
			tempList.Add(SP.ShaderLayersLightingSpecular);
			tempList.Add(SP.ShaderLayersLightingAmbient);
			tempList.Add(SP.ShaderLayersLightingAll);
			tempList.Add(SP.ShaderLayersFragmentPoke);
			tempList.Add(SP.ShaderLayersVertexPoke);
		}
		return tempList;
	}
	...
	public ShaderVar TechQueue = new ShaderVar("Queue",new string[]{"Background", "Geometry", "AlphaTest", "Transparent", "Overlay"},new string[]{"","","","",""},new string[]{"Background", "Geometry", "AlphaTest", "Transparent", "Overlay"});
	public ShaderVar TechReplacement = new ShaderVar("Replacement",new string[]{"Background", "Opaque", "Transparent Cutout", "Transparent", "Overlay", "TreeOpaque", "TreeTransparentCutout", "TreeBillboard","Grass","GrassBillboard"},new string[]{"","","","","","","","","",""},new string[]{"Background", "Opaque", "Transparent Cutout", "Transparent", "Overlay","TreeOpaque","TreeTransparentCutout","TreeBillboard","Grass","GrassBillboard"});
	...
	public string GCShells(ShaderGenerate SG){
		...
		if (ShellsOn.On==true)
		{
			for (int i = 1;i<=ShellsCount.Float;i+=1){
				ShaderCode+=GCPassMask(SG,ShaderPassOLD.ShellBase,(float)i/ShellsCount.Float);
				ShaderCode+=GCPass(SG,ShaderPassOLD.ShellBase,(float)i/ShellsCount.Float);
			}
		}
		return ShaderCode;
	}
	public string GCSubShader(ShaderGenerate SG){
		...
		string Tags = "\"RenderType\"=\""+TechReplacement.CodeNames[TechReplacement.Type]+"\"";
		if (TechQueueAuto.On){
			TechQueue.Type = 1;
			foreach (ShaderPass SP in ShaderPasses){
				if (SP.TransparencyOn.On&&SP.TransparencyType.Type==0)
				TechQueue.Type = 2;
			}
			foreach (ShaderPass SP in ShaderPasses){
				if (SP.TransparencyOn.On&&SP.TransparencyType.Type==1)
				TechQueue.Type = 3;
			}
		}
		Tags+=" \"Queue\"=\""+TechQueue.CodeNames[TechQueue.Type]+"\"";
		if (SG.UsedPoke)
			Tags +="\"SSPoke\" = \"Yes\" ";
	...
	public bool ShaderPassBase(ShaderPassOLD SP){
		if (SP == ShaderPassOLD.Base||SP == ShaderPassOLD.ShellBase||ShaderPassMask(SP))
		return true;
		return false;
	}
	public bool ShaderPassLight(ShaderPassOLD SP){
		if (SP == ShaderPassOLD.Light||SP == ShaderPassOLD.ShellLight)
		return true;
		return false;
	}
	public bool ShaderPassStandard(ShaderPassOLD SP){
		if (SP == ShaderPassOLD.Base||SP == ShaderPassOLD.Light)
		return true;
		return false;
	}
	public bool ShaderPassShells(ShaderPassOLD SP){
		if (SP == ShaderPassOLD.ShellBase||SP == ShaderPassOLD.ShellLight)
		return true;
		return false;
	}
	public bool ShaderPassMask(ShaderPassOLD SP){
		if (SP == ShaderPassOLD.MaskBase||SP == ShaderPassOLD.MaskLight||SP == ShaderPassOLD.Mask)
		return true;
		return false;
	}
	public string GCPass(ShaderGenerate SG,ShaderPassOLD SP){
		return GCPass_Real(SG,SP,0f);
	}
	public string GCPass(ShaderGenerate SG,ShaderPassOLD SP,float Dist){
		return GCPass_Real(SG,SP,Dist);
	}
	public string GCPass_Real(ShaderGenerate SG,ShaderPassOLD SP,float Dist){
		string ShaderCode = "";
		//SG.Reset();
//		int OldTransType = TransparencyType.Type;
	...
	public string GCPassMask(ShaderGenerate SG,ShaderPassOLD SP){
		if (TransparencyZWrite.On&&TransparencyOn.On&&TransparencyType.Type==1)
		return GCPassMask_Real(SG,SP,0f);
		return "";
	}
	public string GCPassMask(ShaderGenerate SG,ShaderPassOLD SP,float Dist){
		if (TransparencyZWrite.On&&TransparencyOn.On&&TransparencyType.Type==1)
		return GCPassMask_Real(SG,SP,Dist);
		return "";
	}
	public string GCPassMask_Real(ShaderGenerate SG,ShaderPassOLD SP,float Dist){
		string ShaderCode = "";
		//SG.Reset();
		int OldTransType = TransparencyType.Type;
//		float OldTransAmount = TransparencyAmount.Float;
...
//			string UvClipName2 = "";
	...
	public string GCFragment(ShaderGenerate SG, ShaderPassOLD SP){
		return GCFragment(SG,SP,0f);
	}
	public string GCFragmentMask(ShaderGenerate SG, ShaderPassOLD SP){
		return GCFragmentMask(SG,SP,0f);
	}
	public string GCFragmentMask(ShaderGenerate SG, ShaderPassOLD SP,float Depth){
		return " fixed4 frag() : SV_Target {\n"+
			"    return fixed4(1.0,0.0,0.0,1.0);\n"+
			"}\n";
	}
	public string GCFragment(ShaderGenerate SG, ShaderPassOLD SP,float Depth){
		string ShaderCode = "";
...
if (!SG.Wireframe){
	...
	public string GCLightingName(ShaderPassOLD SP){
	...
	public string GCPragma(ShaderGenerate SG,ShaderPassOLD SP)
	{
		string shaderCode = "";
		if (ParallaxOn.On)
...
";
	...
	public string GCVertexInternal(ShaderGenerate SG, ShaderPassOLD SP, float Dist){
		string shaderCode = "";
	...
	public string GCVertex(ShaderGenerate SG, ShaderPassOLD SP, float Dist){
		SG.InVertex = true;
		string shaderCode = "";
...
";
	...
	public string GCVertexMask(ShaderGenerate SG, ShaderPassOLD SP, float Dist){
		SG.InVertex = true;
		string shaderCode = "";
	...
	public string GCLighting(ShaderGenerate SG,ShaderPassOLD SP){
		string shaderCode = "";
		string TempStr1;
		string TempStr2;
		...
		if (GCLightingName(SP)=="CLPBR_Standard"){
			...
			CustomPartA = GCLayers(SG,"LightingIndirect",ShaderLayersLightingAmbient,"gi.diffuse","rgb","",false,true).Replace("123ASD$#%","gi.diffuse");
			ShaderLayersLightingAll.NameUnique.Text="LightingDirect";
...
shaderCode+="\n//Generate lighting code similar to the Unity Standard Shader. Not gonna deny, I have no clue how much of it works.\n"+
	...
	public void GenerateUsed(ShaderGenerate SG){
		...
		foreach (ShaderVar SV in ShaderUtil.GetAllShaderVars()){
			if (SV.Obj!=null){
				SG.UsedMasks[((ShaderLayerList)SV.Obj)]++;
			}
		}
		foreach (ShaderInput SI in ShaderInputs){
		...
		foreach (ShaderLayer SL in ShaderUtil.GetAllLayers()){
			...
			//UnityEngine.Debug.Log("WentAhead");
				if (SG.UsedPlugins.IndexOf(ShaderSandwich.PluginList["LayerType"][SL.RealLayerType.Text])<0)
				SG.UsedPlugins.Add(ShaderSandwich.PluginList["LayerType"][SL.RealLayerType.Text]);
				...
				if (SL.MapType.Type==(int)ShaderMapType.UVMap2){
					...
					if (SL.CustomData.D.ContainsKey("Texture")&&SL.CustomData.D["Texture"].Input!=null)
						SL.CustomData.D["Texture"].Input.UsedMapType1 = true;
				}
				if (SL.MapType.Type==(int)ShaderMapType.UVMap1){
					SG.UsedMapType0 = true;
					if (SL.CustomData.D.ContainsKey("Texture")&&SL.CustomData.D["Texture"].Input!=null)
						SL.CustomData.D["Texture"].Input.UsedMapType0 = true;
				}
...
//			UnityEngine.Debug.Log(Line);
	...
	public void OldLoad(Dictionary<string,ShaderVar> D, StringReader S){
		...
		var SP0D = SPBase.GetSaveLoadDict();
		var SP1D = SPShell.GetSaveLoadDict();
		while(1==1){
			string Line = S.ReadLine();
			if (Line!=null){
				if(Line=="EndShaderBase")break;
				...
				if (Line.Contains("#!")){
					ShaderUtil.LoadLine(D,Line);
					ShaderUtil.LoadLine(SP0D,Line);
					ShaderUtil.LoadLine(SP1D,Line);
				}
				else if (Line=="BeginShaderLayerList"){
					Line = S.ReadLine();
					ShaderLayerList.Load(S,((Line.IndexOf("Shell")<0)?SPBase:SPShell).GetShaderLayerLists(),ShaderLayersMasks,Line);
				}
				else if (Line=="BeginShaderInput")
					ShaderInputs.Add(ShaderInput.Load(S));
			}
...
//		UnityEngine.Debug.Log(SB.FileVersion.ToString());
		...
		if (SB.FileVersion<2f){
			EditorUtility.DisplayDialog("Old File","This file was saved with an older version of Shader Sandwich, so it has to be converted to the new format. It might not look exactly the same, sorry!","Ok");
			SB.OldLoad(D,S);
		}else{
			SB.NewLoad(D,S);
		}
	...
	public bool UsesShellLighting(ShaderPassOLD SP){
		if (ShaderPassShells(SP)){
