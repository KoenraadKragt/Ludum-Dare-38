...
public class ShaderPreview : EditorWindow {
	...
	public enum LightSetups{Standard,HighContrast};
	public LightSetups LightSetup = LightSetups.HighContrast;
	...
	public static void Init () {
		...
		windowG.CurCube = windowG.defaultBackCube;
...
//			Debug.Log(MipmapLevel);
	...
	public void Ser(){
		...
		defaultBackCube = ShaderSandwich.KitchenCube;
		#endif
		if (EditorApplication.isPlaying&&EditorApplication.isPlayingOrWillChangePlaymode){//In Play Mode
			Setup();
	...
	void Setup(){
		...
		if ((previewCam==null||previewObject==null||previewCamPoint==null||previewCamPointTarget==null||backMat==null||previewBack==null||Light1==null||Light2==null))
		{
...
//			Debug.Log("CreatedMat");
		...
		if (previewMat==null){//||previewMat.shader==null)
			previewMat2 = (Material)AssetDatabase.LoadAssetAtPath("Assets/ElectronicMindStudios/Shader Sandwich/Internal/Shader Sandwich/SSTemp.mat",typeof(Material));
			if (previewMat2==null){
				...
				if (previewMat2!=null){
					AssetDatabase.CreateAsset(previewMat2, "Assets/ElectronicMindStudios/Shader Sandwich/Internal/Shader Sandwich/SSTemp.mat");
					AssetDatabase.ImportAsset("Assets/ElectronicMindStudios/Shader Sandwich/Internal/Shader Sandwich/SSTemp.mat");
				}
			}
			if (previewMat2!=null){
				if (Shader.Find("Hidden/SSTemp")!=null)
	...
	void OnGUI(){
		...
		EMindGUI.MakeTooltip(1,MeshTypeRect,"The object to preview.\nYou can select a custom one to the right.");
		//Debug.Log(MeshTypeRect);
		//Debug.Log(Event.current.mousePosition);
		//Debug.Log(EMindGUI.Tooltip);
		GUIStyle ButtonStyle = new GUIStyle(GUI.skin.button);
		//if (ShaderSandwich.Instance.CurInterfaceStyle == InterfaceStyle.Modern)
		ButtonStyle = EMindGUI.SCSkin.window;
		MeshType = GUI.SelectionGrid(MeshTypeRect,MeshType,new string[]{"Cube","Sphere","Monkey","Tesselated Cube"},2,ButtonStyle);
		if (MeshType!=-1){
		...
		EMindGUI.MakeTooltip(1,SmoothDampRect,"Make rotating and zooming\nin the viewport smooth.");
		if (SmoothDamp)
		...
		GUI.backgroundColor = OldColb;
			...
			EMindGUI.MakeTooltip(1,ExposeRect,"Expose the temp shader as one you can\nuse in a material (Shader Sandwich/SSTemp).");
			/*if (Expose)
		...
		//}
		//GUILayout.EndHorizontal();
		if (EMindGUI.MouseDownIn(new Rect(0,PreviewRect.y-3,WinSize.x,6),false))
		InResize = true;
...
//previewBack.GetComponent<MeshRenderer>().enabled = false;
		...
		if (!ShaderSandwich.Instance.RealtimePreviewUpdates){
			...
			EMindGUI.Label(new Rect(PreviewRect.x+1,PreviewRect.y+1+PreviewRect.height-20,PreviewRect.width,PreviewRect.height),"Realtime Updates are turned off...",15);
			EMindGUI.Label(new Rect(PreviewRect.x-1,PreviewRect.y-1+PreviewRect.height-20,PreviewRect.width,PreviewRect.height),"Realtime Updates are turned off...",15);
			EMindGUI.Label(new Rect(PreviewRect.x,PreviewRect.y-1+PreviewRect.height-20,PreviewRect.width,PreviewRect.height),"Realtime Updates are turned off...",15);
			EMindGUI.Label(new Rect(PreviewRect.x,PreviewRect.y+1+PreviewRect.height-20,PreviewRect.width,PreviewRect.height),"Realtime Updates are turned off...",15);
			EMindGUI.Label(new Rect(PreviewRect.x-1,PreviewRect.y+PreviewRect.height-20,PreviewRect.width,PreviewRect.height),"Realtime Updates are turned off...",15);
			EMindGUI.Label(new Rect(PreviewRect.x+1,PreviewRect.y+PreviewRect.height-20,PreviewRect.width,PreviewRect.height),"Realtime Updates are turned off...",15);
			GUI.color = new Color(1,0,0,1);
			EMindGUI.Label(new Rect(PreviewRect.x,PreviewRect.y+PreviewRect.height-20,PreviewRect.width,PreviewRect.height),"Realtime Updates are turned off...",15);
			GUI.color = OldCol;
