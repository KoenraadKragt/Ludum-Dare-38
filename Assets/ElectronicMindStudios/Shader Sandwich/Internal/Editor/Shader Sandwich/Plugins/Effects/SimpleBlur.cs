...
public class SSESimpleBlur : ShaderEffect{
	...
	public static string GenerateBase(ShaderEffect SE,ShaderLayer SL,string PixCol,string Map){
		if (SL.RealLayerType.Text == "SLTTexture"){
			PixCol = "tex2Dlod("+SL.CustomData.D["Texture"].Get()+",float4("+Map+",0,"+SE.Inputs[0].Get()+"))";
		}
		if (SL.RealLayerType.Text == "SLTCubemap"){
			PixCol = "texCUBElod("+SL.CustomData.D["Cubemap"].Get()+",float4("+Map+","+SE.Inputs[0].Get()+"))";		
		}
		if (SL.RealLayerType.Text == "SLTGrabPass"){
			PixCol = "tex2Dlod( _GrabTexture, float4("+Map+",0,"+SE.Inputs[0].Get()+"))";
		}
		if (SL.RealLayerType.Text == "SLTDepthPass"){
			if (SL.CustomData.D["Linearize"].On)
			PixCol = "(LinearEyeDepth(tex2Dlod(_CameraDepthTexture, float4("+Map+",0,"+SE.Inputs[0].Get()+")).r).rrrr)";
			else
