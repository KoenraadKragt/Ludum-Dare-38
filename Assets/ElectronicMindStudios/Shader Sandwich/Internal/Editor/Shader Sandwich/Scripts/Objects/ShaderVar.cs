...
public class ShaderVar{
	...
	public string Get(float Wrap){
		...
		if (SG!=null&&SG.Temp&&ShaderSandwich.Instance!=null&&ShaderSandwich.Instance.ImprovedUpdates&&((CType==Types.Float&&!NoInputs)||(CType==Types.Vec)))
		...
		return bs+Float.ToString()+be;
		else
		if (CType==Types.Float)
		return bs+(Float%Wrap).ToString()+be;
	...
	{
		PluginSelectionType = s;
		CType = Types.PluginSelection;
		Plugin = s2;
		Name = Nam;
...
*/
	...
	public void UseEditingMouse(bool Paint){
		...
		if (EditingPopup>0f){
			...
			!(new Rect(rect.x+PopupWidth,rect.y-(20*EditingPopup)-20,PopupWidth,40).Contains(Event.current.mousePosition))
			)){
			...
			if (Paint==true){
				...
				//Debug.Log(rect);
					...
					ShaderUtil.BeginGroup(new Rect(rect.x+PopupWidth,rect.y-(20*EditingPopup)-20,PopupWidth+10,40),GUI.skin.button);
					...
					if (GUI.Button(new Rect(PopupWidth-50,20,15,15),ShaderSandwich.Plus,ButtonStyle)||EMindGUI.MouseUpIn(new Rect(PopupWidth-50,20,15,15))){
						AddInput();
						ShaderSandwich.ShaderVarEditing = null;
					...
					if (GUI.Button(new Rect(PopupWidth-30,20,15,15),ShaderSandwich.GearLinked,ButtonStyle)||EMindGUI.MouseUpIn(new Rect(PopupWidth-30,20,15,15))){
						AddInput();
						ShaderSandwich.ShaderVarEditing = null;
						ShaderSandwich.ValueChanged = true;
						GUI.changed = true;
						...
						ShaderLayerList Mask = ShaderSandwich.Instance.OpenShader.ShaderLayersMasks[0];
						...
						if (Input.Type==3||Input.Type==4){
							ShaderLayer Sl = ShaderLayer.CreateInstance<ShaderLayer>();
							Mask.Add(Sl);
							Sl.CustomData.D["Color"] = new ShaderVar("Color",new Vector4(0f,0f,0f,0f));
					...
					if (GUI.Button(new Rect(PopupWidth-10,20,15,15),ShaderSandwich.CrossRed,ButtonStyle)||EMindGUI.MouseUpIn(new Rect(PopupWidth-10,20,15,15))){
	...
	bool Draw_Real(Rect rect,DrawTypes d,string S)
	{
		...
		if (d == DrawTypes.Type&&Names!=null&&Descriptions!=null&&Type<Descriptions.Length)
		{
			if (ImagePaths==null||ImagePaths.Length==0){
				...
				ButtonStyle = EMindGUI.SCSkin.window;//new GUIStyle(ShaderSandwich.SCSkin.window);
