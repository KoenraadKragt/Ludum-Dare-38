...
float SLTDot2D(float2 P,float3 Rad)
{
...
float SLTDot3D(float3 P,float3 Rad)
{
	P.z+=0.5;
	float3 Pi = floor(P);
	float3 Pf = P-Pi;
...
";
	...
	override public string Generate(Dictionary<string,ShaderVar> Data, ShaderLayer SL, ShaderGenerate SG, string Map){
		...
		if (Data["Noise Dimensions"].Type==0)
			PixCol = "SLTDot2D("+Map+"*3,float3("+Data["MinSize"].Get()+","+Data["MaxSize"].Get()+","+f+"))";
		if (Data["Noise Dimensions"].Type==1)
			PixCol = "SLTDot3D("+Map+"*3,float3("+Data["MinSize"].Get()+","+Data["MaxSize"].Get()+","+f+"))";
		return PixCol;
	}
